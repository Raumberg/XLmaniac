...
from src.namespaces.enums import Phones, Clients, PhoneEnum, Register
...
class PhoneParser(Decoder):
    ...
    def process_phones(self) -> pd.DataFrame:
        ...
        if Phones.TYPE.value in self.df.columns: # переделать эту х полностью, prepare не работает, parse только при prepare
            temporary_dataframe = self.df[[Register.ID.value] + self.columns + [Phones.TYPE.value]].copy()
